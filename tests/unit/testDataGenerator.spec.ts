...
test.describe('Test data generator', () => {
  test('generates the requested total word count without underscores', () => {
    const value = generateTestString('Otomasyon Kampanya', 4, 6);
  ...
  test('normalizes spaces in the starting text', () => {
    expect(generateTestString('  Otomasyon   Kampanya  ', 2, 5)).toBe('Otomasyon Kampanya');
  });

  test('rejects an invalid total word count', () => {
    expect(() => generateTestString('Otomasyon Kampanya', 1, 5)).toThrow('totalWordCount en az 2 olmali.');
  });
});
